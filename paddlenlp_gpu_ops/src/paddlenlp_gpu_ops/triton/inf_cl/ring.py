...
class InfProb(paddle.autograd.PyLayer):
    ...
    def forward(ctx, q, k, group):
        if group is None:
            ...
            # topo = hcg._topo
            # dp_group = topo.get_comm_list("data")
            # for group in dp_group:
            #     print(group)
            dp_group = dist.fleet.get_hybrid_communicate_group().get_data_parallel_group()
            sharding_group = dist.fleet.get_hybrid_communicate_group().get_sharding_parallel_group()
            if dp_group.rank != -1 and sharding_group.rank != -1:
                group = sharding_group
            elif dp_group.rank == -1:
                group = sharding_group
            elif sharding_group.rank == -1:
                group = dp_group
            print(f"dp_group: {dp_group}")
            print(f"sharding_group: {sharding_group}")
            print(f"group: {group}")
        k = k.contiguous()
        comm = RingComm(group)
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        loss1_i2t = cal_inf_loss(q1, k1)
        loss1_t2i = cal_inf_loss(k1, q1)
